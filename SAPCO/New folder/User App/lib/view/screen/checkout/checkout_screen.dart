...
import 'package:sreeleathers/data/model/body/order_place_model.dart';
import 'package:sreeleathers/data/model/response/cart_model.dart';

import 'package:sreeleathers/helper/price_converter.dart';
...
import 'package:sreeleathers/view/basewidget/amount_widget.dart';
...
import 'package:sreeleathers/view/basewidget/my_dialog.dart';
import 'package:sreeleathers/view/basewidget/title_row.dart';
import 'package:sreeleathers/view/screen/cart/cart_screen.dart';
import 'package:sreeleathers/view/screen/checkout/widget/address_bottom_sheet.dart';
import 'package:sreeleathers/view/screen/checkout/widget/custom_check_box.dart';
import 'package:sreeleathers/view/screen/checkout/widget/shipping_method_bottom_sheet.dart';
import 'package:sreeleathers/view/screen/dashboard/dashboard_screen.dart';
import 'package:sreeleathers/view/screen/payment/payment_screen.dart';
...
class _CheckoutScreenState extends State<CheckoutScreen>
{
	...
	void initState()
	{
		...
		Provider.of<OrderProvider>(context, listen: false).initShippingList(context);
									...
									(
										...
										(
											onPressed: () async
											{
												...
												else if(Provider.of<OrderProvider>(context, listen: false).shippingIndex == null)
												{
												...
												else
												{
                      								...
                      								for(int index=0; index < widget.cartList.length; index++)
													{
														...
														(
															...
															(
																...
																'amount',
																index == 0
															  		? Provider.of<OrderProvider>(context, listen: false).shippingList
																		[
																			Provider.of<OrderProvider>(context, listen: false).shippingIndex
																		].id : 1, 
																		cart.variant, 
																		cart.variation != null
																			? [cart.variation]
													...
													(
														...
														(
                        									...
                        									carts, order.paymentMethodIndex == 0 ? 'cash' : '', couponDiscount,
                      									),
											...
											child: Text
											(
												getTranslated('proceed', context), style: titilliumSemiBold.copyWith
												(
													...
													[
														...
														(
															...
															[
																Text
																(
																	Provider.of<OrderProvider>(context).addressIndex == null 
								...
								(
									...
									(
										...
										[
											TitleRow
											(
												title: getTranslated('ORDER_DETAILS', context), onTap: widget.fromProductDetails ? null : () 
												{
												...
												(
													...
													[
														...
														(
															...
															image: '${Provider.of<SplashProvider>(context,listen: false).baseUrls.productThumbnailUrl}/${widget.cartList[0].image}',
															fit: BoxFit.cover,
															width: 50,
															height: 50,
														),
																	...
																	(
																		...
																		[
																			...
																			(
																				...
																				child: Text
																				(
																					PriceConverter.percentageCalculation(context, widget.cartList[0].price, widget.cartList[0].discount, widget.cartList[0].discountType),
																					style: titilliumRegular.copyWith(fontSize: Dimensions.FONT_SIZE_EXTRA_SMALL, color: ColorResources.getPrimary(context)),
																				),
																			),
																		]
													...
													onPressed: () 
													{
														if(_controller.text.isNotEmpty) 
														{
															Provider.of<CouponProvider>(context, listen: false).initCoupon(_controller.text, _order).then((value) 
															{
																if(value > 0) 
																{
																	ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text('You got ${PriceConverter.convertPrice(context, value)} discount'), backgroundColor: Colors.green));
																}
																else 
																{
								...
								(
									...
									(
										...
										[
											...
											CustomCheckBox(title: getTranslated('digital_payment', context), index: 1),
										]
									),
								)
							]
	...
	void _callback(bool isSuccess, String message, String orderID, List<CartModel> carts) async 
	{
    	if(isSuccess) 
		{
			...
			else 
			{
				...
				Navigator.pushReplacement(context, MaterialPageRoute(builder: (_) => PaymentScreen(orderID: orderID, customerID: userID)));
      		}
