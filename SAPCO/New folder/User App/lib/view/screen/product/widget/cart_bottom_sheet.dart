...
import 'package:sreeleathers/data/model/response/product_model.dart';
...
import 'package:sreeleathers/view/screen/checkout/checkout_screen.dart';
...
class CartBottomSheet extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    Provider.of<ProductDetailsProvider>(context, listen: false).initData(product);
          ...
          child: Consumer<ProductDetailsProvider>(
            builder: (context, details, child) {
              ...
              for(int index=0; index < product.choiceOptions.length; index++) {
                _variationList.add(product.choiceOptions[index].options[details.variationIndex[index]].trim());
              }
              ...
              return Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                ...
                Row(children: [
                  ...
                  Expanded(
                    child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                      ...
                      Text(
                        PriceConverter.convertPrice(context, product.unitPrice, discountType: product.discountType, discount: product.discount),
                        ...
                        itemBuilder: (context, i) {
                          return InkWell(
                            ...
                            child: Container(
                              ...
                              padding: EdgeInsets.symmetric(horizontal: Dimensions.PADDING_SIZE_EXTRA_SMALL),
                              decoration: BoxDecoration(
                                color: details.variationIndex[index] != i ? Theme.of(context).accentColor : ColorResources.getPrimary(context),
                                borderRadius: BorderRadius.circular(5),
                              ...
                              child: Text(product.choiceOptions[index].options[i], maxLines: 1, overflow: TextOverflow.ellipsis, style: titilliumRegular.copyWith(
                                ...
                                color: details.variationIndex[index] != i ? Theme.of(context).hintColor : Theme.of(context).accentColor,
                              )),
                            ),
                          );
                        },
                ...
                // Cart button
                CustomButton(
                  buttonText: getTranslated(_stock < 1 ? 'out_of_stock' : isBuy ? 'buy_now' : 'add_to_cart', context),
                  onTap: _stock < 1 ? null : () {
                    if(_stock > 0) {
                      CartModel _cartModel = CartModel(
                        product.id, product.thumbnail, product.name,
                        product.addedBy == 'seller' ? '${Provider.of<SellerProvider>(context, listen: false).sellerModel.fName} '
                            '${Provider.of<SellerProvider>(context, listen: false).sellerModel.lName}' : 'admin',
                        price, priceWithDiscount, details.quantity, _stock, product.colors.length > 0 ? product.colors[details.variantIndex].name : '',
                        _variation, product.discount, product.discountType, product.tax, product.taxType, 1,
                      );
